...
export class UserService {
  ...
  public unsubscribe(login : string, token: string, password : string){
    return this.http.get(this.apiURL+"unsubscribe?login="+login+"&password="+password+"&token="+token);
  }

  public logout(login : string, token : string){
    return this.http.get(this.apiURL+"disconnect?login="+login+"&token="+token);
  }

  public getIconUrl(email : string){
    ...
    return this.gravatarURL+"avatar/"+ md5(email.trim().toLowerCase()).toLowerCase();
  }
